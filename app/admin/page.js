  ...
  const orderColumns = [
    {
      ...
      render: (v, row) => (
        <Link
          ...
          className="font-mono text-coffee-600 hover:underline font-bold text-xs"
          dir="ltr"
        >
          #{v}
        </Link>
    ...
    },
    {
      ...
      title: "مبلغ",
      render: (v) => (
        <span className="font-semibold text-gray-800">{formatPrice(v)}</span>
      ),
    },
    {
      key: "createdAt",
      title: "تاریخ",
      render: (v) => (
        <span className="text-gray-400 text-xs">{formatDate(v)}</span>
      ),
    },
    {
      key: "_id",
      ...
      render: (v) => (
        <Link
          href={`/admin/orders/${v}`}
          className="p-1.5 hover:bg-gray-100 rounded-lg inline-flex text-gray-400 hover:text-gray-700 transition-colors"
        >
          <Eye size={14} />
        </Link>
      ),
        ...
        <div className="grid sm:grid-cols-3 gap-4">
          <div className="bg-white rounded-2xl shadow-card p-5">
            ...
            <p className="text-2xl font-black text-gray-900">
              {formatPrice(stats?.sales?.thisMonth || 0)}
            </p>
          </div>
          <div className="bg-white rounded-2xl shadow-card p-5">
            <p className="text-xs text-gray-400 mb-1">سفارشات این ماه</p>
            <p className="text-2xl font-black text-gray-900">
              {formatNumber(stats?.orders?.thisMonth || 0)}
            </p>
          </div>
          <div className="bg-white rounded-2xl shadow-card p-5">
            <p className="text-xs text-gray-400 mb-1">نظرات در انتظار تأیید</p>
            <p className="text-2xl font-black text-gray-900">
              {formatNumber(stats?.reviews?.pending || 0)}
            </p>
            {stats?.reviews?.pending > 0 && (
              <Link
                href="/admin/reviews"
                className="text-xs text-coffee-600 hover:underline mt-1 inline-block"
              >
                بررسی نظرات ←
              </Link>
            )}
            ...
            <div className="space-y-2">
              {stats.products.lowStock.map((p) => {
                const lowStockVariants =
                  p.variants?.filter((v) => v.stock > 0 && v.stock <= 5) || [];
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-5 gap-3">
            {stats.topProducts.map((p, i) => (
              ...
              >
                ...
                <p className="text-xs text-coffee-600 font-bold">
