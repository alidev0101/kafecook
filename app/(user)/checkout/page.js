...
import { MapPin, Truck, Tag, CreditCard, ShieldCheck, Plus, CheckCircle } from "lucide-react";
...
export default function CheckoutPage() {
  ...
  const shippingCost = subtotal >= 500000 || subtotal === 0 ? 0 : Number(selectedShipping?.price || 35000);

  const finalTotal = Math.max(0, subtotal + shippingCost - Number(discountAmount || 0));
  ...
  const couponMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast.success(
        `کد تخفیف اعمال شد — ${formatPrice(data.discountAmount)} تخفیف`
      );
    },

    onError: (err) =>
      toast.error(
        err.response?.data?.message || "کد تخفیف نامعتبر"
      ),
  });

  const orderMutation = useMutation({
    mutationFn: (data) =>
      axios.post("/api/orders", data).then((r) => r.data.data),
    ...
    onSuccess: async (order) => {
      if (paymentMethod == "online") {
        ...
        router.push(
          `/payment-result?orderId=${order.orderId}&orderNumber=${order.orderNumber}&status=cod`
        );
      }
    },
  ...
  return (
    <div className="bg-background min-h-screen">
      <div className="container-custom py-8">
        ...
        <h1 className="text-2xl font-bold text-foreground mb-7 mt-2">
          تسویه‌حساب
        </h1>
              ...
              {addrLoading ? (
                ...
                <div className="space-y-3">
                  {addresses.map((addr) => (
                    <label
                      key={addr._id}
                      className={`flex items-start gap-3 p-4 rounded-xl border cursor-pointer transition-all ${selectedAddress === addr._id
                    ...
                    >
                      ...
                      <div className="flex-1 text-sm">
                        <p className="font-semibold text-foreground mb-0.5">
                          ...
                          {addr.isDefault && (
                            <span className="mr-2 text-xs text-coffee-500 font-normal">
                              پیش‌فرض
                            </span>
                          )}
                        </p>
                      ...
                      {selectedAddress === addr._id && (
                        <CheckCircle
                          size={18}
                          className="text-coffee-500 flex-shrink-0 mt-0.5"
                        />
                      )}
                    </label>
                  ))}
            ...
            >
              <div className="space-y-2">
                {SHIPPING_METHODS.map((method) => (
                  <label
                    key={method.value}
                    className={`flex items-center justify-between p-4 rounded-xl border cursor-pointer transition-all ${shippingMethod === method.value
                      ? "border-coffee-500 bg-coffee-50 dark:bg-coffee-900/20 dark:border-coffee-400"
            ...
            >
              <div className="space-y-2">
                {[
                  ...
                  <label
                    key={m.value}
                    className={`flex items-center gap-3 p-4 rounded-xl border cursor-pointer transition-all ${paymentMethod === m.value
                      ? "border-coffee-500 bg-coffee-50 dark:bg-coffee-900/20 dark:border-coffee-400"
                      : "border-border hover:border-coffee-300"
                      }`}
                  >
