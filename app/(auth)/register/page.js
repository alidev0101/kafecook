...
export default function RegisterPage() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      toast.success("ثبت‌نام موفق! کد تأیید ارسال شد");

      router.push(
        `/verify-otp?email=${encodeURIComponent(data.email)}&type=register`
      );
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      <p className="text-center text-sm text-coffee-400 mt-6">
        ...
        <Link
          href="/login"
          className="text-coffee-200 hover:text-white font-medium transition-colors"
        >
          وارد شوید
        </Link>
      </p>
